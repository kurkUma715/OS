...
KeyEvent keyboard_read_key(void)
{
    ...
    if (b == 0xE0)
    {
        ...
        {
        case 0x4B:
            event.is_left_arrow = true;
            return event;
        case 0x4D:
            event.is_right_arrow = true;
            return event;
        }
        return event;
    }
    ...
    char c = scancode_lower_case[b];
    if (shift)
    {
        c = scancode_upper_case[b];
    }
    if (!c)
    {
        event.ctrl_pressed = ctrl;
...
char keyboard_read_key_char(bool *is_left_arrow, bool *is_right_arrow)
{
    ...
    if (is_left_arrow)
        *is_left_arrow = event.is_left_arrow;
    if (is_right_arrow)
        *is_right_arrow = event.is_right_arrow;
